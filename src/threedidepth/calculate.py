...
class MyCalculator:
    ...
    def cell_info(self, indexX, coords, centres, lines_wrt_cells, AU, cell_id):
        ...
        if indexX.size == 0:                                 # no cell at position X
            cell_to_X_coords = [self.no_data_value,self.no_data_value,self.no_data_value,self.no_data_value]
            ...
            if indexX.size == 1:
                ...
                for j in range(indexX.size):
                    line_cell_to_X = np.where(((lines_wrt_cells[0,:] == cell_to_X_id[j]) & (lines_wrt_cells[1,:] == cell_id)) | ((lines_wrt_cells[0,:] == cell_id) & (lines_wrt_cells[1,:] == cell_to_X_id[j])))
                    if line_cell_to_X[0].size > 0:            
    ...
    def subcell_info(self, index, cell_coords, cell_centre, subcell, subnb_indices, subnb_centres):
        """ 
        Return:
        ...
        if subcell == 1: 
            sub_coords = [cell_coords[0], cell_coords[1], cell_centre[0], cell_centre[1]]
        elif subcell == 2:
            sub_coords = [cell_centre[0], cell_coords[1], cell_coords[2], cell_centre[1]]
        elif subcell == 3: 
            sub_coords = [cell_centre[0], cell_centre[1], cell_coords[2], cell_coords[3]]
        else: 
            sub_coords = [cell_coords[0], cell_centre[1], cell_centre[0], cell_coords[3]]
        return sub_coords, tri_indices, tri_vertices
    
    def subsubcell_info(self, index, sub_centre, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubcell):
        """ 
        Return:
    ...
    def find_points_subcell(self, pts_in_cell, sub_coords, index_pts_in_cell):
        ...
        """
        index_pts_in_sub = np.where((pts_in_cell[:,0] >= sub_coords[0]) & (pts_in_cell[:,0] <= sub_coords[2]) & (pts_in_cell[:,1] >= sub_coords[1]) & (pts_in_cell[:,1] <= sub_coords[3]))
    ...
    def X_neighbour(self, X, cell_coords, cell_centre, coords):
        ...
        """
        if X == 'l':
            index = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[3] == coords[3,:]))
        elif X == 'r':
            index = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] == coords[3,:]))
        elif X == 'b':
            index = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[1] == coords[3,:]))
        else:
            index = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[3] < coords[3,:]))
        if index[0].size == 0:  # search for smaller neighbours
            if X == 'l':        # search for lower/upper left
                index_small_1 = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_centre[1] == coords[3,:]))
                index_small_2 = np.where((cell_coords[0] > coords[0,:]) & (cell_centre[1] == coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[3] == coords[3,:]))
            elif X == 'r':      # search for lower/upper right
                index_small_1 = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] > coords[3,:]))
                index_small_2 = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] < coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] == coords[3,:]))
            elif X == 'b':      # search for lower left/right
                index_small_1 = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] > coords[2,:]) & (cell_coords[1] == coords[3,:]))
                index_small_2 = np.where((cell_coords[0] < coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[1] == coords[3,:]))
            else:               # search for upper left/right
                index_small_1 = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] > coords[2,:]) & (cell_coords[3] < coords[3,:]))
                index_small_2 = np.where((cell_centre[0] == coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[3] < coords[3,:]))          
            if not (index_small_1[0].size == 0) or not (index_small_2[0].size == 0):
                type_nb = 1     # smaller neighbour exists
                if not (index_small_1[0].size == 0) and not (index_small_2[0].size == 0):
                ...
                if X == 'l':    # lower/upper large cell
                    index_large_1 = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[3] == coords[3,:]))
                    index_large_2 = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[3] < coords[3,:]))
                elif X == 'r':  # lower/upper large cell
                    index_large_1 = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] == coords[3,:]))
                    index_large_2 = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] == coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] < coords[3,:]))
                elif X == 'b':  # left/right large cell
                    index_large_1 = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[1] == coords[3,:]))
                    index_large_2 = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[1] == coords[3,:]))
                else:           # left/right large cell
                    index_large_1 = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] == coords[2,:]) & (cell_coords[3] < coords[3,:]))
                    index_large_2 = np.where((cell_coords[0] == coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] < coords[3,:]))
    ...
    def find_neighbours_subcell(self, subcell, cell_centre, nb_type, nb_indices, nb_coords, nb_centres, nb_au):
        """ 
        ...
        if subcell == 1:
            index_l, coords_l, centre_l, au_l, type_l = self.find_neighbours_subcell_assist('l', nb_indices[0], nb_coords[0], nb_centres[0], nb_au[0], **kwargs_assist)
            index_b, coords_b, centre_b, au_b, type_b = self.find_neighbours_subcell_assist('b', nb_indices[1], nb_coords[1], nb_centres[1], nb_au[1], **kwargs_assist)
            return [index_l, index_b], [coords_l, coords_b], [centre_l, centre_b], [au_l, au_b], np.array([type_l, type_b])
        elif subcell == 2:
            index_b, coords_b, centre_b, au_b, type_b = self.find_neighbours_subcell_assist('b', nb_indices[1], nb_coords[1], nb_centres[1], nb_au[1], **kwargs_assist)
            index_r, coords_r, centre_r, au_r, type_r = self.find_neighbours_subcell_assist('r', nb_indices[2], nb_coords[2], nb_centres[2], nb_au[2], **kwargs_assist)
            return [index_b, index_r], [coords_b, coords_r], [centre_b, centre_r], [au_b, au_r], np.array([type_b, type_r])
        elif subcell == 3:
            index_r, coords_r, centre_r, au_r, type_r = self.find_neighbours_subcell_assist('r', nb_indices[2], nb_coords[2], nb_centres[2], nb_au[2], **kwargs_assist)
            index_t, coords_t, centre_t, au_t, type_t = self.find_neighbours_subcell_assist('t', nb_indices[3], nb_coords[3], nb_centres[3], nb_au[3], **kwargs_assist)
            return [index_r, index_t], [coords_r, coords_t], [centre_r, centre_t], [au_r, au_t],  np.array([type_r, type_t])
        else:
            index_t, coords_t, centre_t, au_t, type_t = self.find_neighbours_subcell_assist('t', nb_indices[3], nb_coords[3], nb_centres[3], nb_au[3], **kwargs_assist)
            index_l, coords_l, centre_l, au_l, type_l = self.find_neighbours_subcell_assist('l', nb_indices[0], nb_coords[0], nb_centres[0], nb_au[0], **kwargs_assist)
            return [index_t, index_l], [coords_t, coords_l], [centre_t, centre_l], [au_t, au_l], np.array([type_t, type_l])           

    def find_neighbours_subcell_assist(self, X, indexX, cell_to_X_coords, cell_to_X_centre, au_cell_to_X, subcell, cell_centre, nb_type):
        """ 
        ...
        if indexX.size > 1:             # cell has two smaller neighbours
            if subcell == subcell1 or subcell == subcell2:
                if subcell == subcell1:
                    location = np.where(cell_to_X_centre[row,:] == np.min(cell_to_X_centre[row,:]))[0][0]
                else:
                    location = np.where(cell_to_X_centre[row,:] == np.max(cell_to_X_centre[row,:]))[0][0]
            ...
            if nb_type[n_t_index] == 1: # cell has 1 smaller neighbour
                if (subcell == subcell1 and cell_to_X_centre[row] <= cell_centre[row]) or (subcell == subcell2 and cell_to_X_centre[row] >= cell_centre[row]):
                    index_X = indexX
                ...
                single_cell_to_X_coords = cell_to_X_coords
                type_X = nb_type[n_t_index]
        return index_X, single_cell_to_X_coords, single_cell_to_X_centre, single_au_cell_to_X, type_X
    ...
    def refine_cell(self, cell_coords, cell_centre):
        ...
        lb_centre = [lb_coords[0]+(lb_coords[2]-lb_coords[0])/2, lb_coords[1]+(lb_coords[3]-lb_coords[1])/2]

        # right_bottom:
        rb_coords = [cell_centre[0], cell_coords[1], cell_coords[2], cell_centre[1]]
        rb_centre = [rb_coords[0]+(rb_coords[2]-rb_coords[0])/2, rb_coords[1]+(rb_coords[3]-rb_coords[1])/2]

        # right_top:
        rt_coords = [cell_centre[0], cell_centre[1], cell_coords[2], cell_coords[3]]
        rt_centre = [rt_coords[0]+(rt_coords[2]-rt_coords[0])/2, rt_coords[1]+(rt_coords[3]-rt_coords[1])/2]

        # left_top:
        lt_coords = [cell_coords[0], cell_centre[1], cell_centre[0], cell_coords[3]]
        lt_centre = [lt_coords[0]+(lt_coords[2]-lt_coords[0])/2, lt_coords[1]+(lt_coords[3]-lt_coords[1])/2]
    ...
    def find_containing_cell(self, fine_coords, centre):
        ...
        return fine_coords[np.where((centre[0] > fine_coords[:,0]) & (centre[0] < fine_coords[:,2]) & (centre[1] > fine_coords[:,1]) & (centre[1] < fine_coords[:,3])),:][0][0]

    def bary_lin_interpolator(self, pts_in_sub, tri_indices, modified_Matrix, waterlevels):
        """ 
        ...
        C = np.array([waterlevels[tri_indices[0]], waterlevels[tri_indices[1]], waterlevels[tri_indices[2]]], dtype = np.ndarray)
    ...
    def triangle_IDSW_interpolator(self, pts_in_sub, waterlevels, tri_indices, tri_vertices):
        """ 
...
class Improvement1(MyCalculator): 
    ...
    def compute_nr_neighbours(self, index, waterlevels, au, Matrix):
        ...
        for j in range(3):
            i = i+1
            if ((not (j==1))  and ((index[j].size == 0) or (waterlevels[index[j]] == self.no_data_value))) or ((j == 0) and (au[0] < 10**(-7))) or ((j==2) and (au[1] < 10**(-7))):
    ...
    def determine_interpolation_type(self, tri_indices, waterlevels, au, Matrix):
        ...
        nr_nbs, modified_Matrix, water = self.compute_nr_neighbours(tri_indices, waterlevels, au, Matrix)
        if nr_nbs == 0:
    ...
    def pseudo_interpolation(self, index, cell_centre, cell_coords, tri_indices, tri_vertices, subcell, waterlevels, pts_in_sub):
        """ 
        ...
        if subcell == 1:
            ...
            for j in range(3):
                if tri_indices[j].size > 0:
                    ...
                    if j == 2:
                        pseudo_coords[j] = [cell_centre[0] - dx, cell_centre[1]]        
        Matrix = [[pseudo_coords[0][0], pseudo_coords[1][0], pseudo_coords[2][0]], [pseudo_coords[0][1], pseudo_coords[1][1], pseudo_coords[2][1]], [1, 1, 1]]
    ...
    def Central_Interpolator(self, pts_in_sub, waterlevels, tri_indices, tri_vertices, au, Matrix):
        """ 
        ...
        interpolation_type, water, modified_Matrix = self.determine_interpolation_type(tri_indices, waterlevels, au, Matrix) 
        if interpolation_type == 1:
            ...
            if INTERPOLATION_METHOD == 'barycentric':
                interpolated = self.bary_lin_interpolator(pts_in_sub, tri_indices, modified_Matrix, waterlevels)
            elif INTERPOLATION_METHOD == 'distance':
                interpolated = self.triangle_IDSW_interpolator(pts_in_sub, waterlevels, tri_indices, tri_vertices)
            return interpolated
    ...
    def refine_neighbours(self, subnb_types, subnb_coords, subnb_centres):
        ...
        if subnb_types[0] == 0:                             # same size, refine once
            nb1_fine_coords, nb1_fine_centres = self.refine_cell(subnb_coords[0], subnb_centres[0])
            centre_nb1 = self.find_closest(nb1_fine_centres, centre_nb2)
        if subnb_types[1] == 0:                             # same size, refine once
            nb2_fine_coords, nb2_fine_centres = self.refine_cell(subnb_coords[1], subnb_centres[1])
            centre_nb2 = self.find_closest(nb2_fine_centres, centre_nb1)
        if subnb_types[0] == 2:                             # larger, refine twice
            nb1_fine_coords1, nb1_fine_centres1 = self.refine_cell(subnb_coords[0], subnb_centres[0])
            ...
            nb1_fine_coords, nb1_fine_centres = self.refine_cell(coords_nb11, centre_nb11)
            centre_nb1 = self.find_closest(nb1_fine_centres, centre_nb2)
        if  subnb_types[1] == 2:                            # larger, refine twice
            nb2_fine_coords1, nb2_fine_centres1 = self.refine_cell(subnb_coords[1], subnb_centres[1])
            ...
            nb2_fine_coords, nb2_fine_centres = self.refine_cell(coords_nb21, centre_nb21)
            centre_nb2 = self.find_closest(nb2_fine_centres, centre_nb1)
        return centre_nb1, centre_nb2

    def small_neighbour_interpolator(self, index, cell_coords, cell_centre, sub_coords, sub_centre, waterlevels, subnb_indices, subnb_coords, subnb_centres, subnb_au, subcell, subnb_types, pts_in_sub, index_pts_in_sub, result):
        """ 
        ...
        centre_nb1, centre_nb2 = self.refine_neighbours(subnb_types, subnb_coords, subnb_centres)
        ...
        if subcell == 1:
            ...
            subsubnbs_centres = [centre_nb1, centre_nb2, fine_centres[1], fine_centres[3]]
            subsubnbs_au = [subnb_au[0], subnb_au[1], 1, 1]
        elif subcell == 2:
            subsubnbs_indices = [index, subnb_indices[0], subnb_indices[1], index]
            subsubnbs_centres = [fine_centres[0], centre_nb1, centre_nb2, fine_centres[2]]
            subsubnbs_au = [1, subnb_au[0], subnb_au[1], 1]
        elif subcell == 3:
            subsubnbs_indices = [index, index, subnb_indices[0], subnb_indices[1]]
            subsubnbs_centres = [fine_centres[3], fine_centres[1], centre_nb1, centre_nb2]
            subsubnbs_au = [1, 1, subnb_au[0], subnb_au[1]]
        else:
            subsubnbs_indices = [subnb_indices[1], index, index, subnb_indices[0]]
            subsubnbs_centres = [centre_nb2, fine_centres[0], fine_centres[2],centre_nb1]
            subsubnbs_au = [subnb_au[1], 1, 1, subnb_au[0]]

        for subsubcell in range(1,5):
            subsub_coords = sub_fine_coords[subsubcell-1]
            subtri_vertices, subtri_indices, subau = self.subsubcell_info(index, sub_centre, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubcell)
            pts_in_subsub, subindex_pts = self.find_points_subcell(pts_in_sub, subsub_coords, index_pts_in_sub)
            if pts_in_subsub.size == 0:
                continue
            subMatrix = [[subtri_vertices[0][0],subtri_vertices[1][0],subtri_vertices[2][0]],[subtri_vertices[0][1],subtri_vertices[1][1],subtri_vertices[2][1]],[1,1,1]]
            subinterpolated = self.Central_Interpolator(pts_in_subsub, waterlevels, subtri_indices, subtri_vertices, subau, subMatrix)
            result[subindex_pts] = subinterpolated
        return result

    def large_neighbour_interpolator(self, index, cell_centre, pts_in_sub, subnb_types, subnb_indices, subnb_coords, subnb_centres, waterlevels, au):        
        """ 
        ...
        tri_indices = np.array([np.array([subnb_indices[0]]), index, np.array([subnb_indices[1]])], dtype = np.ndarray)
        Matrix = [[tri_vertices[0][0],tri_vertices[1][0],tri_vertices[2][0]],[tri_vertices[0][1],tri_vertices[1][1],tri_vertices[2][1]],[1,1,1]]
        interpolated = self.Central_Interpolator(pts_in_sub, waterlevels, tri_indices, tri_vertices, au, Matrix)
        return interpolated
    ...
    def __call__(self, indices, values, no_data_value):  
        ...
        pt_to_cell = self._get_nodgrid(indices).reshape(nr_points)     # array giving for each point in which cell it is.
        ...
        for cell_id in indexrange:
            ...
            if waterlevels[index] == no_data_value: # check if is waterlevel available for cell
                result[index_pts_in_cell] = no_data_value
                continue
                
            kwargs_neighbour = dict(cell_coords = cell_coords, cell_centre = cell_centre, coords = coords)
            kwargs_info = dict(coords = coords, centres = centres, lines_wrt_cells = lines_wrt_cells, AU = AU, cell_id = cell_id)
            # cell to left
            index_left, type_left = self.X_neighbour('l', **kwargs_neighbour)
            coords_left, centre_left, au_left = self.cell_info(index_left, **kwargs_info)         
            
            # cell to right
            index_right, type_right = self.X_neighbour('r', **kwargs_neighbour)
            coords_right, centre_right, au_right = self.cell_info(index_right, **kwargs_info)

            # cell to bottom
            index_bottom, type_bottom = self.X_neighbour('b', **kwargs_neighbour)
            coords_bottom, centre_bottom, au_bottom = self.cell_info(index_bottom, **kwargs_info)

            #cell to top
            index_top, type_top = self.X_neighbour('t', **kwargs_neighbour)
            coords_top, cell_to_top_centre, au_top = self.cell_info(index_top, **kwargs_info)
            
            nb_indices = [index_left, index_bottom, index_right, index_top]
            nb_coords = [coords_left, coords_bottom, coords_right, coords_top]
            nb_centres = [centre_left.copy(), centre_bottom.copy(), centre_right.copy(), cell_to_top_centre.copy()]
            ...
            for subcell in range(1,5):
                cell_centre = cell_centre_true.copy()
                # info subcell
                subnb_indices, subnb_coords, subnb_centres, subnb_au, subnb_types = self.find_neighbours_subcell(subcell, cell_centre, nb_type, nb_indices, nb_coords, nb_centres, nb_au)
                sub_coords, tri_indices, tri_vertices = self.subcell_info(index, cell_coords, cell_centre, subcell, subnb_indices, subnb_centres)
                sub_centre = [sub_coords[0]+(sub_coords[2]-sub_coords[0])/2, sub_coords[1]+(sub_coords[3]-sub_coords[1])/2]
                
                # find interpolating points in each subcel
                pts_in_sub, index_pts = self.find_points_subcell(pts_in_cell, sub_coords, index_pts_in_cell)
                if pts_in_sub.size == 0:                    # no points in subcell
                    continue
                
                if np.any(subnb_types == 1):                # at least 1 neighbour smaller
                    result = self.small_neighbour_interpolator(index, cell_coords, cell_centre, sub_coords, sub_centre,  waterlevels, subnb_indices, subnb_coords, subnb_centres, subnb_au, subcell, subnb_types, pts_in_sub, index_pts, result)
                elif np.any(subnb_types == 2):              # at least 1 neighbour larger
                    interpolated = self.large_neighbour_interpolator(index, cell_centre, pts_in_sub, subnb_types, subnb_indices, subnb_coords, subnb_centres, waterlevels, subnb_au)
                    result[index_pts] = interpolated
                else:                                       # all same size or no neighbours
                    Matrix = [[tri_vertices[0][0], tri_vertices[1][0], tri_vertices[2][0]], [tri_vertices[0][1], tri_vertices[1][1], tri_vertices[2][1]], [1, 1, 1]]
                    interpolated = self.Central_Interpolator(pts_in_sub, waterlevels, tri_indices.copy(), tri_vertices, subnb_au, Matrix)
                    result[index_pts] = interpolated
...
class Improvement2(MyCalculator):   
    ...
    def X_diagonal_neighbour(self, X, cell_coords, cell_centre, coords, centres, waterlevels, subnb_indices, lines_wrt_cells): 
        """ 
        ...
        """
        if X == 'lb':
            indexX = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[1] == coords[3,:]))
        elif X == 'rb':
            indexX = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[1] > coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[1] == coords[3,:]))
        elif X == 'rt':
            indexX = np.where((cell_coords[2] == coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[2] < coords[2,:]) & (cell_coords[3] < coords[3,:]))
        else:
            indexX = np.where((cell_coords[0] > coords[0,:]) & (cell_coords[3] == coords[1,:]) & (cell_coords[0] == coords[2,:]) & (cell_coords[3] < coords[3,:]))      
        
        line_cell_to_X = [-1, -1]
        sub_nbs_ids = [-1, -1]
        if (indexX[0].size == 0) or (waterlevels[indexX] == self.no_data_value):    # no neighbour with known water level
            indexX = np.array([[]])
            centreX = [self.no_data_value, self.no_data_value]
            coordsX = [self.no_data_value, self.no_data_value,self.no_data_value, self.no_data_value]
            typeX = 3
        else:
            cell_to_X_id = indexX[0][0] + 1
            sub_nbs_ids[0] = subnb_indices[0] + 1
            sub_nbs_ids[1] = subnb_indices[1] + 1
            line_cell_to_X[0] = np.where(((lines_wrt_cells[0,:] == cell_to_X_id) & (lines_wrt_cells[1,:] == sub_nbs_ids[0])) | ((lines_wrt_cells[0,:] == sub_nbs_ids[0]) & (lines_wrt_cells[1,:] == cell_to_X_id)))
            line_cell_to_X[1] = np.where(((lines_wrt_cells[0,:] == cell_to_X_id) & (lines_wrt_cells[1,:] == sub_nbs_ids[1])) | ((lines_wrt_cells[0,:] == sub_nbs_ids[1]) & (lines_wrt_cells[1,:] == cell_to_X_id)))
            if line_cell_to_X[0][0].size < 1 or line_cell_to_X[1][0].size < 1:      # no line between orthogonal and diagonal neighbour
                indexX = np.array([[]])
                centreX = [self.no_data_value, self.no_data_value]
                coordsX = [self.no_data_value, self.no_data_value,self.no_data_value, self.no_data_value]
                typeX = 3
            else:
                if cell_coords[2]-cell_coords[0] == coords[2,indexX]-coords[0,indexX]:          # same size
                    centreX = centres[indexX,:][0][0]
                    coordsX = coords[:,indexX[0][0]]
                    typeX = 0
                elif 1/2*(cell_coords[2]-cell_coords[0]) == coords[2,indexX]-coords[0,indexX]:  # 1x smaller
                    centreX = centres[indexX,:][0][0]
                    coordsX = coords[:,indexX[0][0]]
                    typeX = 1
                elif 2*(cell_coords[2]-cell_coords[0]) == coords[2,indexX]-coords[0,indexX]:    # 1x larger
                    centreX = centres[indexX,:][0][0]
                    coordsX = coords[:,indexX[0][0]]
                    typeX = 2
                elif 1/4*(cell_coords[2]-cell_coords[0]) == coords[2,indexX]-coords[0,indexX]:  # 2x smaller, coarsen diagonal neighbour
                    centreX = centres[indexX,:][0][0]
                    coordsX = coords[:,indexX[0][0]]
                    if X == 'lb':
                    ...
                    typeX = 1
                elif 4*(cell_coords[2]-cell_coords[0]) == coords[2,indexX]-coords[0,indexX]:    # 2x larger, refine diagonal neighbour
                    centreX = centres[indexX,:][0][0]
                    coordsX = coords[:,indexX[0][0]]
    ...
    def orth_nb_as_diag_nb(self, nb_type, sub_nb_index, nb_index, nb_au, lines_wrt_cells, AU):
        """
        ...
        if nb_type == 0 or nb_type == 2:    # same size/larger orthogonal neighbours
            ...
            if nb_index.size > 1:           # 2 smaller orhtogonal neighbours
                ...
                if np.all(nb_au > 0):
                    line = np.where(((lines_wrt_cells[0,:] == nb_index[0]+1) & (lines_wrt_cells[1,:] == nb_index[1]+1)) | ((lines_wrt_cells[0,:] == nb_index[1]+1) & (lines_wrt_cells[1,:] == nb_index[0]+1)))
                    au = AU[line]   
                    if au > 0:
                        return indexX  
        return np.array([[]])

    def find_diag_neighbours_subcell(self, index, subcell, sub_coords, sub_centre, nb_indices, ori_nb_au, subnb_indices, subnb_types, dnb_index, lines_wrt_cells, AU):
        """ 
        Return indices and coordinates of centres of four diagonal neighbours of subcell.
        """
        sub_dnb_indices = [np.array([[]]),np.array([[]]),np.array([[]]),np.array([[]])]
        sub_dnb_centres = [[self.no_data_value, self.no_data_value], [self.no_data_value, self.no_data_value], [self.no_data_value, self.no_data_value], [self.no_data_value, self.no_data_value]]
        subnb_indices = np.array(subnb_indices, dtype = np.ndarray)
        dx = 2*(sub_centre[0] - sub_coords[0])
        dy = 2*(sub_centre[1] - sub_coords[1])
        sub_dnb_centres = [[sub_centre[0]-dx, sub_centre[1]-dy], [sub_centre[0]+dx, sub_centre[1]-dy], [sub_centre[0]+dx, sub_centre[1]+dy], [sub_centre[0]-dx, sub_centre[1]+dy]]

        if subcell == 1:
            sub_dnb_indices[0] = dnb_index
            sub_dnb_indices[1] = self.orth_nb_as_diag_nb(subnb_types[1], subnb_indices[1], nb_indices[1], ori_nb_au[1], lines_wrt_cells, AU)
            sub_dnb_indices[2] = index
            sub_dnb_indices[3] = self.orth_nb_as_diag_nb(subnb_types[0], subnb_indices[0], nb_indices[0], ori_nb_au[0], lines_wrt_cells, AU)
        elif subcell == 2:
            sub_dnb_indices[0] = self.orth_nb_as_diag_nb(subnb_types[0], subnb_indices[0], nb_indices[1], ori_nb_au[1], lines_wrt_cells, AU)
            sub_dnb_indices[1] = dnb_index
            sub_dnb_indices[2] = self.orth_nb_as_diag_nb(subnb_types[1], subnb_indices[1], nb_indices[2], ori_nb_au[2], lines_wrt_cells, AU)
            sub_dnb_indices[3] = index
        elif subcell == 3:
            sub_dnb_indices[0] = index
            sub_dnb_indices[1] = self.orth_nb_as_diag_nb(subnb_types[0], subnb_indices[0], nb_indices[2], ori_nb_au[2], lines_wrt_cells, AU)
            sub_dnb_indices[2] = dnb_index
            sub_dnb_indices[3] = self.orth_nb_as_diag_nb(subnb_types[1], subnb_indices[1], nb_indices[3], ori_nb_au[3], lines_wrt_cells, AU)
        elif subcell == 4:
            sub_dnb_indices[0] = self.orth_nb_as_diag_nb(subnb_types[1], subnb_indices[1], nb_indices[0], ori_nb_au[0], lines_wrt_cells, AU)
            sub_dnb_indices[1] = index
            sub_dnb_indices[2] = self.orth_nb_as_diag_nb(subnb_types[0], subnb_indices[0], nb_indices[3], ori_nb_au[3], lines_wrt_cells, AU)
    ...
    def check_diag_water(self, indexX, typeX, nb_indices, lines_wrt_cells, AU, nb_au):
        ...
        """
        if indexX[0].size > 0 and nb_indices[0].size > 0 and nb_au[0] > 10**(-7) and nb_indices[1].size > 0 and nb_au[1] > 10**(-7):
            line1 = np.where(((lines_wrt_cells[0,:] == nb_indices[0]+1) & (lines_wrt_cells[1,:] == indexX[0]+1)) | ((lines_wrt_cells[0,:] == indexX[0]+1) & (lines_wrt_cells[1,:] == nb_indices[0]+1)))
            line2 = np.where(((lines_wrt_cells[0,:] == nb_indices[1]+1) & (lines_wrt_cells[1,:] == indexX[0]+1)) | ((lines_wrt_cells[0,:] == indexX[0]+1) & (lines_wrt_cells[1,:] == nb_indices[1]+1)))
            au1 = AU[line1]
            au2 = AU[line2]
    ...
    def determine_interpolation_type(self, tri_indices, indexD, waterlevels, au, Matrix):
        """ 
        Return:
            interpolation_type: type to be used based on the number and type of neighbours
                (0 = error, cell has no known waterlevel, should not interpolate,
        ...
        if waterlevels[tri_indices[1]] == self.no_data_value:
            interpolation_type = 0
            print("error: interpolation type 0")
        nr_nbs, modified_Matrix, water = self.compute_nr_neighbours(tri_indices, indexD, waterlevels, au, Matrix)
        if nr_nbs == 0:
            interpolation_type = 1    
        elif nr_nbs == 1:
            interpolation_type = 2    
    ...
    def Central_Interpolator(self, pts_in_sub, waterlevels, tri_indices, tri_vertices, au, Matrix, square_vertices, square_indices):
        """ 
        ...
        """
        nr_points = len(pts_in_sub)
        interpolation_type, water, modified_Matrix = self.determine_interpolation_type(tri_indices, square_indices[3], waterlevels, au, Matrix) 
        ...
        if interpolation_type == 1:
            ...
            if INTERPOLATION_METHOD == 'barycentric':
                interpolated = self.bilinear_interpolator(pts_in_sub, square_vertices, square_indices, waterlevels)
            elif INTERPOLATION_METHOD == 'distance':
                interpolated = self.square_IDSW_interpolator(pts_in_sub, square_vertices, square_indices, waterlevels)
            elif INTERPOLATION_METHOD == 'combi':
                interpolated = self.bilinear_interpolator(pts_in_sub, square_vertices, square_indices, waterlevels)
            return interpolated
    ...
    def find_diag_neighbour_centre(self, dnb_type, dnb_centre, dnb_coords, sub_centre):
        ...
        if dnb_type == 2:                   # larger, refine twice
            dnb_fine_coords1, dnb_fine_centres1 = self.refine_cell(dnb_coords, dnb_centre)
            ...
            dnb_fine_coords, dnb_fine_centres = self.refine_cell(coords_dnb1, centre_dnb1)
            centre_dnb = self.find_closest(dnb_fine_centres, sub_centre)
        return centre_dnb

    def small_neighbour_prep(self, index, cell_coords, cell_centre, nb_indices, nb_au, subcell, sub_coords, sub_centre, subnb_indices, subnb_coords, subnb_centres, subnb_au, subnb_types, dnb_index, dnb_type, dnb_centre, dnb_coords, lines_wrt_cells, AU):
        """ 
        Return:
        ...
        centre_nb1, centre_nb2 = self.refine_neighbours(subnb_types, subnb_coords, subnb_centres)
        sub_fine_coords, sub_fine_centres = self.refine_cell(sub_coords, sub_centre)
        centre_dnb = self.find_diag_neighbour_centre(dnb_type, dnb_centre, dnb_coords, sub_centre)
        subsubdnbs_indices, subsubdnbs_centres = self.find_diag_neighbours_subcell(index, subcell, sub_coords, sub_centre, nb_indices, nb_au, subnb_indices, subnb_types, dnb_index, lines_wrt_cells, AU)
        subnb_indices = np.array(subnb_indices, dtype = np.ndarray)
        if subcell == 1:
            subsubnbs_indices = [subnb_indices[0], subnb_indices[1], index, index]
            subsubnbs_centres = [centre_nb1, centre_nb2, fine_centres[1], fine_centres[3]]
            ...
            subsubnbs_centres = [centre_nb2, fine_centres[0], fine_centres[2],centre_nb1]
            subsubnbs_au = [subnb_au[1], 1, 1, subnb_au[0]]
        return sub_fine_coords, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubdnbs_indices, subsubdnbs_centres

    def small_neighbour_interpolator(self, index, sub_centre, waterlevels, pts_in_sub, index_pts_in_sub, sub_fine_coords, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubdnbs_indices, subsubdnbs_centres, result):
        """ 
        ...
        for subsubcell in range(1,5):
            ...
            if pts_in_subsub.size == 0:
                continue
            subMatrix = [[subtri_vertices[0][0],subtri_vertices[1][0],subtri_vertices[2][0]],[subtri_vertices[0][1],subtri_vertices[1][1],subtri_vertices[2][1]],[1,1,1]]
            subinterpolated = self.Central_Interpolator(pts_in_subsub, waterlevels, subtri_indices, subtri_vertices, subau, subMatrix, subsquare_vertices, subsquare_indices)
            result[subindex_pts] = subinterpolated
        return result

    def orthog_is_diag(self, subcell, subnb_types, subnb_indices, subnb_centres, subnb_coords, cell_centre, diag_nb_centre, diag_nb_index):
        """ 
    ...
    def large_neighbour_interpolator(self, index, subcell, cell_centre, pts_in_sub, subnb_types, subnb_indices, subnb_coords, subnb_centres, diag_nb_index, diag_nb_type, diag_nb_coords, diag_nb_centre, waterlevels, au):        
        """ 
        ...
        if diag_nb_type == 2:           # large, refine once
            nbd_fine_coords, nbd_fine_centres = self.refine_cell(diag_nb_coords, diag_nb_centre)
            ...
            centre_nbd, diag_nb_index = self.orthog_is_diag(subcell, subnb_types, subnb_indices, subnb_centres, subnb_coords, cell_centre, diag_nb_centre, diag_nb_index.copy())
        else:                           # same size or nonexistent
            centre_nbd = diag_nb_centre
        ...
        tri_indices = np.array([np.array([subnb_indices[0]]), index, np.array([subnb_indices[1]])], dtype = np.ndarray)
        Matrix = [[tri_vertices[0][0],tri_vertices[1][0],tri_vertices[2][0]],[tri_vertices[0][1],tri_vertices[1][1],tri_vertices[2][1]],[1,1,1]]
        square_vertices = [[tri_vertices[0][0], tri_vertices[0][1]], [tri_vertices[1][0], tri_vertices[1][1]], [tri_vertices[2][0], tri_vertices[2][1]], centre_nbd]
        square_indices = [tri_indices[0], tri_indices[1], tri_indices[2], diag_nb_index]               
        interpolated = self.Central_Interpolator(pts_in_sub, waterlevels, tri_indices, tri_vertices, au, Matrix, square_vertices, square_indices)
        return interpolated
   
    def bilinear_interpolator(self, pts_in_sub, square_vertices, square_indices, waterlevels):
        """ 
        Return interpolated (array): bilinearly interpolated water levels at points in subcell.
        """
        C = np.array([waterlevels[square_indices[0]], waterlevels[square_indices[1]], waterlevels[square_indices[2]], waterlevels[square_indices[3]]], dtype = float)
        biMatrix = np.matrix([[1,square_vertices[0][0],square_vertices[0][1],square_vertices[0][0]*square_vertices[0][1]], [1,square_vertices[1][0],square_vertices[1][1],square_vertices[1][0]*square_vertices[1][1]], [1,square_vertices[2][0],square_vertices[2][1],square_vertices[2][0]*square_vertices[2][1]], [1,square_vertices[3][0],square_vertices[3][1],square_vertices[3][0]*square_vertices[3][1]]], dtype = 'float')
        bi_arr = np.c_[np.ones(len(pts_in_sub)), pts_in_sub[:,0], pts_in_sub[:,1], pts_in_sub[:,0]*pts_in_sub[:,1]].transpose()
        b = np.dot((np.linalg.inv(biMatrix)).transpose(), bi_arr)
        interpolated = b.transpose()@C
        return interpolated
   
    def square_IDSW_interpolator(self, pts_in_sub, square_vertices, square_indices, waterlevels):
        """ 
    ...
    def __call__(self, indices, values, no_data_value):  
        ...
        for cell_id in indexrange:
            ...
            for subcell in range(1,5):
                ...
                dnb_index_pre, dnb_centre, dnb_coords, dnb_type = self.X_diagonal_neighbour(diag_loc[subcell-1], cell_coords, cell_centre, coords, centres, waterlevels, subnb_indices, lines_wrt_cells)
                dnb_index, dnb_type = self.check_diag_water(dnb_index_pre, dnb_type, subnb_indices, lines_wrt_cells, AU, subnb_au)
                
                if np.any(subnb_types == 1) or dnb_type == 1:               # at least 1 neighbour smaller
                    sub_fine_coords, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubdnbs_indices, subsubdnbs_centres = self.small_neighbour_prep(index, cell_coords, cell_centre, nb_indices, nb_au, subcell, sub_coords, sub_centre, subnb_indices, subnb_coords, subnb_centres, subnb_au, subnb_types, dnb_index, dnb_type, dnb_centre, dnb_coords, lines_wrt_cells, AU)
                    result = self.small_neighbour_interpolator(index, sub_centre, waterlevels, pts_in_sub, index_pts, sub_fine_coords, subsubnbs_indices, subsubnbs_centres, subsubnbs_au, subsubdnbs_indices, subsubdnbs_centres, result)
                elif np.any(subnb_types == 2) or dnb_type == 2:             # at least 1 neighbour larger
                    interpolated = self.large_neighbour_interpolator(index, subcell, cell_centre, pts_in_sub, subnb_types, subnb_indices, subnb_coords, subnb_centres, dnb_index, dnb_type, dnb_coords, dnb_centre, waterlevels, subnb_au)
                    result[index_pts] = interpolated
                else:                                                       # all same size or no neighbours
                    Matrix = [[tri_vertices[0][0], tri_vertices[1][0], tri_vertices[2][0]], [tri_vertices[0][1], tri_vertices[1][1], tri_vertices[2][1]], [1, 1, 1]]
                    square_vertices = [[tri_vertices[0][0], tri_vertices[0][1]], [tri_vertices[1][0], tri_vertices[1][1]], [tri_vertices[2][0], tri_vertices[2][1]], dnb_centre]
                    square_indices = [tri_indices[0], tri_indices[1], tri_indices[2], dnb_index]
                    interpolated = self.Central_Interpolator(pts_in_sub, waterlevels, tri_indices.copy(), tri_vertices, subnb_au, Matrix, square_vertices, square_indices)
                    result[index_pts] = interpolated
        result = (np.array([result])).reshape(values.shape)
        return result
...
class GeoTIFFConverter:
    ...
    Args:
        ...
        amd 1.0.
    """
    def __init__(self, gridadmin_path, results_3di_path, calculation_step, source_path, target_path, progress_func=None):
